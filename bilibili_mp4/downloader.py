...
class BililiContainer():
    ...
    def __str__(self):
        """ 视频返回的字符串 """
        num_block = len(self.video.blocks)
        return "{} qn:{} {}".format(self.name, self.video.qn, num_block)

    def set_video(self, **kwargs):
        self.video = BililiMedia(container=self, **kwargs)
...
class BililiMedia():
    ...
    def __init__(self, url, qn, container):
    ...
    def _get_head(self):
        res = self.container.spider.head(
            self.url, headers={'Range': 'bytes=0-4'})
...
class BililiBlock():
    ...
    def download(self, ffmpeg, stream=True, chunk_size=1024):
        ...
        if all([block.status.done for block in self.media.blocks]):
            ...
            if self.container.video.status.done:
                self.container.status.switch()
